...
namespace eval IXIA {
	...
	array set m_transmitmode [list]
	#add flag for invoking ixStartPortPacketGroups command
	variable m_startpg 0
	#标识用例是否启动抓包
    array set m_capture_flag [list]
	array set m_trigger_mode [list]
    #从0-based转换成1-based
    #跟Smartbits一致
    ...
    # FE ：100.0
    # 10 ：10.0
    proc SmbGetSpeed {Chas Card Port} {
        set phySpeed 0.0
        port get $Chas $Card $Port
           set phySpeed [port cget -speed]
        return $phySpeed
    }
    proc SmbDeleteIpv4Stack {intHub intSlot intPort args} {
		#删除device
		return 0
	}
    #!!================================================================
    ...
    proc SmbCapPacketLengthGet {Chas Card Port args} {
        ...
        if {($PktCnt > 0) && ($Index < $PktCnt)} {
            #修改最后一个包是要取的包，这样当index比PktCnt小很多的时候，获取报文的速度要快几十秒
            if {[captureBuffer get $Chas $Card $Port 1 [expr $Index + 1]]} {
                set retVal 1
    ...
    proc SmbCapturePktGet {Chas Card Port args} {     
        ...
        if {($PktCnt > 0) && ($Index < $PktCnt)} {            
            ...
            set byteList [list]
            #smb抓的包是不包含crc的，这里要减去4个crc字节
            for {set i $Offset} {$i < [expr $Len -4]} {incr i} {
    ...
    proc SmbCaptureStart {Chas Card Port args} {
        ...
        set ::IXIA::m_capture_flag($Chas,$Card,$Port) 1
        ...
        if { $index >=0 } {
           ...
           if { $trigger == 1 } { 
              ...
              puts "SmbCaptureStart set triger"
               capture setDefault
               #设置抓包后，每个报文保存的最大长度是10000，默认是8191
               capture config -sliceSize 10000
               
               capture config -beforeTriggerFilter captureBeforeTriggerFilter
            ...
            #set IXIA::m_trigger 0
            puts "SmbCaptureStart set no triger"
            capture setDefault
            capture config -beforeTriggerFilter captureBeforeTriggerAll 
            capture config -afterTriggerFilter  captureAfterTriggerAll 
            ...
            #如果第一次设置了trg过滤抓包，第二次没有trg的抓包，如果不设置下面，去掉过滤会不生效（涉及到filter的动作都要下下面的命令）
            lappend portList [list $Chas $Card $Port]
            if [ixWriteConfigToHardware portList -noProtocolServer ] {
                 IxPuts -red "Unable to write configs to hardware!"
                set retVal 1
    ...
    proc SmbCaptureStop {Chas Card Port} {
        set retVal 0
		set ::IXIA::m_capture_flag($Chas,$Card,$Port) 0
        if {[ixStopPortCapture $Chas $Card $Port] != 0} {
            IxPuts -red "Could not start capture on $Chas $Card $Port"
            set retVal 1
        }
    ...
    proc IxChecksumcalc {array1 {start 14} {stop 33} } {
     ...
     proc SmbClosePort {Chas Card Port isClose} {
         ...
         #开端口，轮询查端口等待端口link up
         if {$isClose == "on"} {
            for {set i 1} {$i<6} {incr i} {
                set linkStat [stat getLinkState $Chas $Card $Port]
                puts "Port $Chas $Card $Port linkStat: $linkStat (1 is linkUp)"
                #如果link up则等0.5秒后还是Linkup则退出
                if {$linkStat == 1} {
                    after 500
                    if { [stat getLinkState $Chas $Card $Port] == 1} {
                        break
                    }
                } else {
                    after  500
    ...
    proc SmbCustomPacketSet  {Chas Card Port Len Lcon args}  {
        #这里需要占用一下端口，企业网有些用例是直接构造报文，然后设置发送模式的，
		#如果不占用端口会导致报文内容的更改
		...
		#端口模式下建流先清除流modify by 2012-07-25
        ...
        set ls {}
		set iconLen [llength $Lcon]
		puts "iconLen=====#{iconLen}"
		for {set i 0} {$i < [expr $Len + $iconLen]} {incr i} {
			lappend Lcon "00"
		}
        foreach sub $Lcon {
    ...
    #过程描述：     设置端口错误包类型
    #用法：         
    #示例：         IxErrorSet 1 1 1 -crc 1  等价于 IxErrorSet 1 1 1 -crc
    #               
    #参数说明：     
    #               Chas:      Ixia的hub号
    #               Card:      Ixia接口卡所在的槽号
...
puts "attr:$attr"
    ...
    proc SmbMetricModeSet {Chas Card Port args} {
        set retVal     0
        puts "SmbMetricModeSet $Chas $Card $Port $args"
        if {[IxParaCheck $args] == 1} {
            set retVal 1
            return $retVal
        }
        ...
        while { $tmpllength > 0  } {
            ...
            case $cmdx      {
                ...
                #-mode         {  }
                -error {set crc 1}
                -signature  { puts "signature not support"}
                default     {
                    set retVal 0
                  #  IxPuts -red "Error : cmd option $cmdx does not exist"
                   # return $retVal
        ...
        if  { $metricflag == 1 } {
            ...
            if {$flag == 0} {
			    if {$IXIA::m_transmitmode($Chas,$Card,$Port) == "AdvancedScheduler"} {
					#端口的AdvancedScheduler模式要和流的conpackage组合才能达到均分发流
					set Strtransmode 0
				} else {
        ...
        if {$Clear == 1} {
            ...
            #port write $Chas $Card $Port
			#这里需要把配置写入硬件，否则会导致部分流无法删除的问题（wuwangjing 00109914反馈）
			lappend portList [list $Chas $Card $Port]
			if {[ixWriteConfigToHardware portList -noProtocolServer]} {
			    IxPuts -red "Can't write stream to  $Chas $Card $Port"    
			}  
            ...
            #UDF1 config
			puts "Vfd1len===========$Vfd1len"
			puts "Vfd1offset==========$Vfd1offset"
            if {$Vfd1 != "OffState"} {
                #countertype最多支持32位。对于源mac和目的mac直接在流上配置跳变
				if {$Vfd1len==6 && $Vfd1offset==0} {
					#跳变DsaMac
					stream config -numDA  $Vfd1cycle
					stream config -da  $Vfd1start
					switch $Vfd1 {
						"IncreState" {stream config -daRepeatCounter increment}
					}
					stream config -daStep  $Vfd1step                          
				...
				} elseif {$Vfd1len==6 && $Vfd1offset==6} {
					#跳变SaMac
					stream config -numSA  $Vfd1cycle
					stream config -sa  $Vfd1start
					switch $Vfd1 {
						"IncreState" {stream config -saRepeatCounter increment}
					}
					stream config -saStep $Vfd1step
            ...
            #UDF2 config
			puts "Vfd2len============$Vfd2len"
			puts "Vfd2offset==========$Vfd2offset"
            if {$Vfd2 != "OffState"} {
			    if {$Vfd2len==6 && $Vfd2offset==0} {
					#跳变DsaMac
					stream config -numDA  $Vfd2cycle
					stream config -da  $Vfd2start
					switch $Vfd2 {
						"IncreState" {stream config -daRepeatCounter increment}
					}
					stream config -daStep  $Vfd2step                          
					stream config -daMaskValue  "00 00 00 00 00 00"
					stream config -daMaskSelect "00 00 00 00 00 00"
					stream config -enableSaContinueFromLastValue      false
				} elseif {$Vfd2len==6 && $Vfd2offset==6} {
					#跳变SaMac
					stream config -numSA  $Vfd2cycle
					stream config -sa  $Vfd2start
					switch $Vfd2 {
						"IncreState" {stream config -saRepeatCounter increment}
					}
					stream config -saStep $Vfd2step
					stream config -saMaskValue                        "00 00 00 00 00 00"
					stream config -saMaskSelect                       "00 00 00 00 00 00"
					stream config -enableSaContinueFromLastValue      false
				} else {
...
IxPuts -red $applyStrResult
       ...
       # }
		...
		#add pgid
        if { $Signature == "true"} {
			stream get $Chas $Card $Port $Streamid
			...
			packetGroup setTx $Chas $Card $Port $Streamid 
			if { [ stream write $Chas $Card $Port $Streamid ] } {
				IxPuts -red "Can't write stream $Chas $Card $Port $Streamid"
					set retVal 1
			}
    ...
    proc DebugInfo {Chas Card Port} {
	    lappend portList [list $Chas $Card $Port ]
		set checkowner [ixCheckOwnership $portList]
		puts "checkowner=====$checkowner"
		port get $Chas $Card $portList
    ...
    proc SmbPortClear {Chas Card Port args} {
        ...
        if {[ixClearPortStats $Chas $Card $Port]} {
		    DebugInfo $Chas $Card $Port
		    IxPuts -red "Can't Clear $Chas $Card $Port counters"
            set retVal 1
			return "${retVal},$::ixErrorInfo"
        }
       # lappend portList [list $Chas $Card $Port]
...
#          }        
    ...
    proc SmbPortLinkStatusGet {Chas Card Port } {
        ...
        #return $retVal  
		return [list $retVal  $retVal]
    }
	
	#设置端口物理属性
    proc SmbPhyConfigSet {Chas Card Port args} {
        set errCode 0
        ...
        port get $Chas $Card $Port
        if {[llength $card_mode]} {
            switch -exact -- $card_mode {
                "lan" { 
                    port config -portMode 4
                }
                "wan" { 
                    #port config -portMode 1
                    port config -portMode portEthernetMode
                    #puts "port config -portMode 1"
                }
                default {
                    error "物理模式(-mode)取值错误:${card_mode}"
                }
            }
            if {[port set $Chas $Card $Port]} {
							puts "Error calling port set $Chas $Card $Port"
							set retCode $::TCL_ERROR
         }
			#必须设置完wan后才能设置SDH,这里ixWritePortsToHardware接口调用是必须的
            if [ixWritePortsToHardware portList -noProtocolServer] {
				IxPuts -red "Can't write config to $Chas $Card $Port"
				catch { ixputs $::ixErrorInfo} err
				set retVal 1   
        ...
        if {[llength $fram_mode]} {
            switch -exact -- $fram_mode {
                "sonet" { 
                		#sonet config -header                             sonetHdlcPppIp
										sonet config -interfaceType                      ethOverSonet
                }
                "sdh" { 
                  sonet config -interfaceType                      ethOverSdh
                    
                }
                default {
                    error "帧模式(-framing)取值错误:${fram_mode}"
                }
            }
            if {[sonet set $Chas $Card $Port]} {
				puts "Error calling sonet set "
				set retCode 1
        }
		if [ixWritePortsToHardware portList -noProtocolServer] {
			IxPuts -red "Can't write config to $Chas $Card $Port"
    ...
    proc SmbPortModeSet { Chas Card Port args} {
        ...
        set speed "ALL"
        set autoneg "TRUE"
        #修改默认为AdvancedScheduler模式，否则配置多条流发的是第一条流其他流都都不会被发送，设置这个模式后各条流的流速是均分的
        set TransmitMode "AdvancedScheduler" 
		set ::IXIA::m_transmitmode($Chas,$Card,$Port) $TransmitMode
        set PhyMode "portPhyModeFiber"

        set args [string tolower $args]

        #设置TransmitMode默认值，其他表如smb和stc都没有这个参数的，因此这里要做默认参数设置
        #前面已经设置了TransmitMode默认值，下面代码可以注释
		# if {[string first -transmitmode $args] == -1} {
            # lappend args -transmitmode
            # lappend args $TransmitMode
        # }
        ...
        #这里有问题，恢复出厂模式会把用户配的流删除掉后配置默认的一条流，如果发包过程中动态修改端口工作模式，会导致设置的流丢失
        # # initialize
        ##------------
        #if { [port setFactoryDefaults $Chas $Card $Port] } {
...
#        }
    ...
    switch  $TransmitMode {
        PacketStreams {
		    puts "port config -transmitMode   portTxPacketStreams"
	    set ::IXIA::m_transmitmode($Chas,$Card,$Port) "PacketStreams"
            port config -transmitMode   portTxPacketStreams
	}
        AdvancedScheduler {
	    set ::IXIA::m_transmitmode($Chas,$Card,$Port) "AdvancedScheduler"
            port config -transmitMode   portTxModeAdvancedScheduler
            ##允许各条流的流速不均分，要保证各条的的流速不能大于端口速率
            ##port isValidFeature $Chas $Card $Port portFeatureTransparentDynamic   
        }
    }
    #set ::IXIA::m_transmitmode $TransmitMode
...
#        puts "ixTakeOwnership $Chas $Card $Port $UserName"
    ...
    proc SmbPortRun {Chas Card Port {runPara 2}} {
		...
		#获取端口下根据流的统计
		if {$m_startpg == 0} {
			for {set i 0} {$i< [llength $m_portList]} {incr i} {
				set temChas [lindex [lindex $m_portList $i] 0]
				set temCard [lindex [lindex $m_portList $i] 1]
				set temPort [lindex [lindex $m_portList $i] 2] 
			    ixStopPortPacketGroups $temChas $temCard $temPort
				if { [ixStartPortPacketGroups $temChas $temCard $temPort] != 0 } {
					IxPuts -red "Could not start packet groups on  $temChas: $temCard :$temPort"
					set retVal 1
				}	
			}
		set m_startpg 1
    ...
    proc SmbRcvTrafficGet { Chas Card Port args} {
        set all_counter_flag 0
        set all_counter ""
        if {$args==""} {
            set all_counter_flag 1
            set args {-frame -packet -framerate -byte -byterate -collision -trigger -triggerrate -crc -crcrate -over -overrate -frag}
        }
        ...
        while { $tmpllength > 0  } {
            ...
            case $cmdx      {
                ...
                -byte           {
                    #mali 00245266反馈这里需要调用一下statAllStats
					#否则结果会有异常
					stat get statAllStats $Chas $Card $Port
                    set TempVal  [stat cget -bytesReceived]
                    lappend FinalList $TempVal
                    lappend all_counter  rx_byte $TempVal
                }
                -byterate   {
                    stat getRate allStats $Chas $Card $Port
                    set TempVal  [stat cget -bytesReceived]
                ...
                -undersize {
                    ...
                    lappend FinalList $TempVal
                    lappend all_counter  rx_frag $TempVal
                }
                -trigger    {
				    #在niuchenxiao 00183557统计上调试，统计的时候需要调用statAllStats接口，否则会导致计数统计不到
                    stat get statAllStats $Chas $Card $Port
                    #=============
                    # Modified by Eric Yu
                    # set TempVal  [stat cget -captureFilter]
					set TempVal 0
                  if {[info exists ::IXIA::m_trigger_mode($Chas,$Card,$Port)] && $::IXIA::m_trigger_mode($Chas,$Card,$Port) == 6} {
						#如果trigger是两个条件或的关系，需要两个条件都统计
						set stata1 [stat cget -userDefinedStat1]
						set stata2 [stat cget -userDefinedStat2]
						set TempVal [expr $stata1 + $stata2]
					} else {
                    set TempVal [stat cget -userDefinedStat1]
					}
                    #=============
                    lappend FinalList $TempVal   
                    lappend all_counter  rx_trigger $TempVal             
                }
                -triggerrate    {
                    stat getRate allStats $Chas $Card $Port
                    #=============
                ...
                -overrate   {
                    ...
                    set TempVal  [stat cget -oversize]
                    lappend FinalList $TempVal 
                    lappend all_counter  rx_over_rate $TempVal
                }
                -frag       {
                    #stat get statAllStats $Chas $Card $Port
    ...
    proc SmbSendModeSet  {Chas Card Port enumSendMode args} {
        ...
        #}
        #end of added
        if {[info exists ::IXIA::m_capture_flag($Chas,$Card,$Port)] && $::IXIA::m_capture_flag($Chas,$Card,$Port) == 1} {
			#先停止抓包
        ...
        for { set Index 1 } { $Index <= $streamCnt } { incr Index } {
					...
					##
					set Gelen [ stream cget -framesize ]
...
puts "port transmit mode: $enumSendMode"
                    switch  $enumSendMode {
                        ...
                        SINGLEBURST {
                                #stream config -dma stopStream
								set dmastat [stream cget -dma]
								if {$dmastat == 0 || $dmastat == 1 } {
									stream config -dma stopStream
								}
                                stream config -numBursts 1
                                #stream config -numFrames $Singleburst
                                #smb 上这个操作是端口级的，因此这里不能简单对各条流字节设置突发个数，要重新根据流个数分担下
                                if {$Index != $streamCnt} {
                                    stream config -numFrames [expr ($Singleburst)/($streamCnt)]
                                } else {
                                    stream config -numFrames [expr ($Singleburst)/($streamCnt) + ($Singleburst)%($streamCnt)]
                                }
                        }
                        MULTIBURST  {
                                stream config -dma advance
                                ...
                                if {$Index != $streamCnt} {
                                    stream config -numFrames [expr ($Perburst)/($streamCnt)]
                                } else {
                                    stream config -numFrames [expr ($Perburst)/($streamCnt) + ($Perburst%$streamCnt)%($streamCnt)]
                                }                                
            ...
            #                return $retVal
                    ...
                    if {($Pps == 0) && ($Rate == 0) && ($Bps == 0)} {
                        ...
                        #stream config -percentPacketRate [expr ($Rate)/1.0/($streamCnt)]  
                        #默认是初始速率设置为100%,解决默认由于速率过低导致突发报文无法正常发送的问题
						set Rate 100
						stream config -percentPacketRate [expr ($Rate)/1.0/($streamCnt)]  
                    }
                    ...
                    if { $Rate != 0 } {
                        ...
                        stream config -rateMode streamRateModeFps
                        #stream config -fpsRate $Pps
                        stream config -fpsRate [expr ($Pps)/1.0/($streamCnt)]
                    } elseif { ($Bps != 0) && ($Rate == 0) && ($Pps == 0) } {
                        #用户选择用bps为单位的情况
                        stream config -rateMode streamRateModeBps
                        #stream config -bpsRate $Bps
                        stream config -bpsRate [expr ($Bps)/1.0/($streamCnt)]
                    }
    ...
    proc SmbSendTrafficGet {Chas Card Port args} {
        ...
        if {$args==""} {
            set all_counter_flag 1
            set args {-frame -packet -framerate -pingreply -pingrequest -arpreply -arprequest  -byte -igmptxframe}
        }    
       ...
       # lappend FinalList $retVal
        ...
        while { $tmpllength > 0  } {
            ...
            case $cmdx      {
                -frame {
                    ...
                    lappend FinalList $TempVal
                    lappend all_counter  tx_frames $TempVal
                }
                -byterate {
                    stat getRate allStats $Chas $Card $Port
		...
		-packet   {
                    ...
                    set TempVal  [stat cget -framesSent]
                    lappend FinalList $TempVal
                    lappend all_counter tx_packet $TempVal
                }
                -byte   {
                    stat get statAllStats $Chas $Card $Port
                    set TempVal  [stat cget -bytesSent]
                    lappend FinalList $TempVal
                    lappend all_counter tx_byte $TempVal
                }
                -framerate      {
                    stat getRate allStats $Chas $Card $Port
                    set TempVal [stat cget -framesSent]
                    lappend FinalList $TempVal
                    lappend all_counter tx_rate $TempVal
                }
                -jumborate      {
                    lappend FinalList -1
                    puts "tx_jumborate ixia 不支持"
                    lappend all_counter tx_jumborate -1
                } 
                ...
                -arpreply   {
                    ...
                    lappend FinalList $TempVal
                    lappend all_counter tx_arpreply $TempVal
                }
                -arprequest {
                    stat get statAllStats $Chas $Card $Port
                    set TempVal  [stat cget -txArpRequest]
                    lappend FinalList $TempVal
                    lappend all_counter tx_arprequest $TempVal
                }
                -igmptxframe {
                    #stat get statAllStats $Chas $Card $Port
                    ...
                    #lappend FinalList $TempVal
                    set TempVal  -1
                    puts "tx_igmptxframe ixia 不支持"
                    lappend all_counter tx_igmptxframe $TempVal
                }
                -igmpframe  {
                    #stat get statAllStats $Chas $Card $Port
                    #set TempVal  [stat cget -txIgmpFrames]
                ...
                -pingreply {
                    ...
                    lappend FinalList $TempVal
                    lappend all_counter tx_pingreply $TempVal
                }
                -pingrequest {
                    stat get statAllStats $Chas $Card $Port
                    set TempVal  [stat cget -txPingRequest]
                    lappend FinalList $TempVal
                    lappend all_counter tx_pingrequest $TempVal
                }
                default     {
                    set retVal 1
                    IxPuts -red "Error : cmd option $cmdx does not exist"
    ...
    proc SmbTriggerSet {Chas Card Port enumParttern args} {
        ...
        switch $enumParttern {
            ...
            "TRI1ORTRI2" {
                set TriggerMode 6
			    set ::IXIA::m_trigger_mode($Chas,$Card,$Port) $TriggerMode
            }
        ...
        if {$TriggerMode == 6} {
			#ixia界面上没有TRI1ORTRI2选择的，所以这里把两个Pattern都激活
			#mofify by 2014/06/26
        ...
        filter config -userDefinedStat2Pattern $TriggerMode
        filter config -userDefinedStat2Enable false
		}
        
        #设置抓包触发开关，没有用，因为支撑里都是显示的调用启动抓包的
        filter config -captureTriggerPattern $TriggerMode
        filter config -captureTriggerEnable false
        ...
        #writes streams, filters, protocol configuration to port
        #这里必须要设置下，否则filter不生效
        lappend portList [list $Chas $Card $Port]
        if [ixWriteConfigToHardware portList -noProtocolServer ] {
            IxPuts -red "Unable to write configs to hardware!"
            set retVal 1
...
#        }
    ...
    proc SmbVfdSet {Chas Card Port args} {
        ...
        if {$Vfd1 != "OffState"} {
			...
			if {$Vfd1len==6 && $Vfd1offset==0} {
				...
				switch $Vfd1 {
					"IncreState" {stream config -daRepeatCounter increment}
					"RandomState" {stream config -daRepeatCounter ctrRandom}
				}
				stream config -daStep  $Vfd1step                          
				stream config -daMaskValue  "00 00 00 00 00 00"
				stream config -daMaskSelect "00 00 00 00 00 00"
			   ...
			   #跳变SaMac
				...
				switch $Vfd1 {
					"IncreState" {stream config -saRepeatCounter increment}
					"RandomState" {stream config -saRepeatCounter ctrRandom}
				}
				stream config -saStep $Vfd1step
				stream config -saMaskValue                        "00 00 00 00 00 00"
				stream config -saMaskSelect                       "00 00 00 00 00 00"
   ...
   #!!================================================================
    ...
    #程 序 包：    SmbPL 1.0
    #功能类别：    
    #过程描述：    判断MAC地址形式是否合法,若合法返回1,否则返回0
    #              StrJudgeMac 1.2.3.4             ;#返回值为0
    #              StrJudgeMac .2.3                ;#返回值为0
    #              StrJudgeMac 1.2.q               ;#返回值为0
    #              StrJudgeMac 01.123.123f         ;#返回值为1
    #
    #参数说明：    Mac:  MAC地址(合法形式为:H.H.H)
    #
    #返 回 值：    若合法返回1,否则返回0
    #作    者：    
    #生成日期：    
    #修改纪录：    
    #        -----------------------------------------------------------
    #        时间：2012-6-2 17:09:58
    #        作者：ckf40043
    #!!================================================================ 
   proc StrJudgeMac { Mac } {
        set Temp1 [split $Mac .]
        set Len [llength $Temp1]
        puts "Len===============$Len"
        
        
        for {set i 0} {$i <= 2} {incr i} {
            set  Temp2 [lindex $Temp1 $i]
			puts "Temp2=================$Temp2"
            set MatchResult  [regexp {^([0-9a-fA-F])$|^([0-9a-fA-F][0-9a-fA-F])$|^([0-9a-fA-F][0-9a-fA-F][0-9a-fA-F])$|^([0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F])$} $Temp2]
            if {$MatchResult == 0} {
                return 0
            }
        }
            ...
            switch -glob -- $cmdx {
                ...
                -strtransmode {
                    if {$val == ""} {
                        IxPuts -red "Error : cmd option $cmdx $val is null"
                        set retVal 1
                    } elseif { $val != 0 && $val != 1 && $val != 2 && $val != 3 && $val != 4 && $val != 5} {
    ...
    #
    #!!================================================================
	proc SmbDelIpv4Stack {chassis card port {args ""}} {
		...
		if {$args != ""} {
			#删除指定device
			interfaceTable delInterface $args
	...
	proc SmbModifyIpv4Stack {chassis card port args} {
	    set ip           "0.0.0.0"
        set netmask      "24"
        set gateway      "1.1.1.1"
       ...
       # if {[arpServer set $chassis $card $port]} {
		...
		if { [ info exists name ] } {
			interfaceTable getInterface $name
			#删除device
			#interfaceTable delInterface $name			
		} else {
            errorMsg ""
            set retCode $::TCL_ERROR
		}
    ...
    proc SmbSetIpv4Stack {chassis card port args} {
        ...
        if {![info exists name]} {
		    puts "interfaceTable clearAllInterfaces "
			interfaceTable clearAllInterfaces 
		}
        ...
        foreach {each_gateway} $gateway {each_ip} $ip {each_netmask} $netmask {each_mac} $mac {each_vlan} $vlan {
			...
			if { [info exists name] } {
			    puts "interfaceEntry config -description $name"
				interfaceEntry config -description $name
				#if { [interfaceEntry config -description $name] } {
				#	errorMsg "Error calling interfaceEntry config -description $name"
				#	set retCode $::TCL_ERROR
				#}
			}
...
#       puts $line
    ...
    # FE ：100.0
    # 10 ：10.0
    # proc SmbGetSpeed {intHub intSlot intPort} {
        # port cget -speed
    # }
    ...
    #修改纪录：     
    #!!================================================================
	proc SmbStreamStatsTx {Chas Card Port args } {
		set retVal 0
			...
			switch -exact -- $key {
				-streamid {
					if {[string is integer $value]} {
						...
						error "streamid must be interger"
					}
				}
			}
		}
    ...
    #修改纪录：     
    #!!================================================================
	proc SmbStreamStatsRx {Chas Card Port } {
		set retVal 0
		set maxTxStreams  255
		set maxRxStreams  4096
		ixStopPortPacketGroups $Chas $Card $Port
        if { [ixStartPortPacketGroups $Chas $Card $Port] != 0 } {
		    IxPuts -red "Could not start packet groups on  $Chas: $Card :$Port"
			set retVal 1
		}
    ...
    #修改纪录：     
    #!!================================================================
	proc SmbStreamStats {Chas Card Port args } {
		...
		if {[info exists streamid] } {
		} else { set streamid "all" }

		# ## ensure the port can be used.
		# if { [ port canUse $Chas $Card $Port ] != 1 } {
		
			 # return "Error:please reserve the port first.\n"
		 # }
		array set arr {}
		...
		##Fetchs twice just ensure that the rxByteRate/rxFrameRate/rxBitRate always own an value.
			...
			if {$numStreams == 0} {	
				IxPuts -blue  " No Rx stream on $Chas $Card $Port,please ensure the option of \"wide packet groups\" in port's receive mode is enabled."
			}

			for {set i $numStreams} {$i >= 1} {incr i -1} {
						
				packetGroupStats getGroup $i				
				set arr("\_$Card\_$Port\_$i,rxByteRate")       [packetGroupStats cget -byteRate]
				set arr("\_$Card\_$Port\_$i,rxFrameRate")      [packetGroupStats cget -frameRate]
				set arr("\_$Card\_$Port\_$i,rxBitRate")        [packetGroupStats cget -bitRate]
				set arr("\_$Card\_$Port\_$i,rxTotalFrames")    [packetGroupStats cget -totalFrames]
				set arr("\_$Card\_$Port\_$i,rxTotalBytes")     [packetGroupStats cget -totalByteCount]
				set arr("\_$Card\_$Port\_$i,rxMinLatency")     [packetGroupStats cget -minLatency]
				set arr("\_$Card\_$Port\_$i,rxMaxLatency")     [packetGroupStats cget -maxLatency]
				set arr("\_$Card\_$Port\_$i,rxAverageLatency") [packetGroupStats cget -averageLatency]
				set arr("\_$Card\_$Port\_$i,rxFirstTime")      [packetGroupStats cget -firstTimeStamp]
				set arr("\_$Card\_$Port\_$i,rxLastTime")       [packetGroupStats cget -lastTimeStamp]     
			}
		...
		##Make sure the stream is configured on the specified port.
			...
			for { set i $numStreams} {$i >= 1} {incr i -1} {
				...
				set arr("\_$Card\_$Port\_$i,txByteRate") [expr [streamTransmitStats cget -frameRate]*[stream cget -framesize]]
				set arr("\_$Card\_$Port\_$i,txFrameRate") [streamTransmitStats cget -frameRate]
				set arr("\_$Card\_$Port\_$i,txBitRate")   [expr [streamTransmitStats cget -frameRate]*[stream cget -framesize] *8]
				set arr("\_$Card\_$Port\_$i,txTotalFrames") [streamTransmitStats cget -framesSent]
				set arr("\_$Card\_$Port\_$i,txTotalBytes") [expr [streamTransmitStats cget -framesSent]*8]
				set arr("\_$Card\_$Port\_$i,txLastTime")  [streamTransmitStats cget -lastTimeStamp]
			}
		
		} else { 
		
		## make sure the input have no syntax error or any typo.	
		##Fetch twice just ensure that the Byterate/framesrate/bitrate always own an value.	
			packetGroupStats get $Chas $Card $Port 1 $maxRxStreams
			packetGroupStats get $Chas $Card $Port 1 $maxRxStreams
			
			if { [catch { packetGroupStats getGroup $streamid} ] } {
				IxPuts -blue  " No Rx stream$streamid on $Chas $Card $Port,please ensure the option of \"wide packet groups\" in port's receive mode is enabled."
			} else {

				set arr("\_$Card\_$Port\_$streamid,rxByteRate")       [packetGroupStats cget -byteRate]
				set arr("\_$Card\_$Port\_$streamid,rxFrameRate")      [packetGroupStats cget -frameRate]
				set arr("\_$Card\_$Port\_$streamid,rxBitRate")        [packetGroupStats cget -bitRate]
				set arr("\_$Card\_$Port\_$streamid,rxTotalFrames")    [packetGroupStats cget -totalFrames]
				set arr("\_$Card\_$Port\_$streamid,rxTotalBytes")     [packetGroupStats cget -totalByteCount]
				set arr("\_$Card\_$Port\_$streamid,rxMinLatency")     [packetGroupStats cget -minLatency]
				set arr("\_$Card\_$Port\_$streamid,rxMaxLatency")     [packetGroupStats cget -maxLatency]
				set arr("\_$Card\_$Port\_$streamid,rxAverageLatency") [packetGroupStats cget -averageLatency]
				set arr("\_$Card\_$Port\_$streamid,rxFirstTime")      [packetGroupStats cget -firstTimeStamp]
				set arr("\_$Card\_$Port\_$streamid,rxLastTime")       [packetGroupStats cget -lastTimeStamp]     
			}
			
		
			streamTransmitStats get $Chas $Card $Port 1 $maxRxStreams
			if { [catch { streamTransmitStats getGroup $streamid} ] } {
				IxPuts -blue  " No Tx stream$streamid on $Chas $Card $Port."
			} else {

				set arr("\_$Card\_$Port\_$streamid,txByteRate") [expr [streamTransmitStats cget -frameRate]*[stream cget -framesize]]
				set arr("\_$Card\_$Port\_$streamid,txFrameRate") [streamTransmitStats cget -frameRate]
				set arr("\_$Card\_$Port\_$streamid,txBitRate")   [expr [streamTransmitStats cget -frameRate]*[stream cget -framesize] *8]
				set arr("\_$Card\_$Port\_$streamid,txTotalFrames") [streamTransmitStats cget -framesSent]
				set arr("\_$Card\_$Port\_$streamid,txTotalBytes") [expr [streamTransmitStats cget -framesSent]*8]
				set arr("\_$Card\_$Port\_$streamid,txLastTime")  [streamTransmitStats cget -lastTimeStamp] 
			}
